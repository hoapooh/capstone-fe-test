...
export function useArtistProfileImageUpload(userId: string) {
  ...
  const updateProfileMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["artist", userId] });
    },
    onError: (error) => {
  ...
  const uploadAvatar = async (file: File) => {
    ...
    try {
      ...
      const uploadResult = await uploadImageToCloudinary(file, {
        folder: `artist-profiles/${userId}`,
        tags: ["artist", "profile", "avatar"],
        resourceType: "image",
      });
